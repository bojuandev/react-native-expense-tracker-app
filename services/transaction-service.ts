...
import { getLast12Months, getLast7Days, getYearsRange } from "@/utils/commons";
  ...
  try {
    ...
    if (image) {
      ...
      if (!imageUploadRes.success) {
        return {
          success: false,
          msg: imageUploadRes.msg || "Failed to upload receip",
        };
      }
...
const updateWalletForNewTransaction = async (
  ...
  try {
    ...
    return { success: true, msg: "transaction udated succesfully" };
  } catch (error: any) {
...
export const deleteTransaction = async (
  ...
  try {
    ...
    if (transactionType === "expense" && newWalletAmount < 0) {
      return { success: false, msg: "You can not delet this transaction" };
    }
